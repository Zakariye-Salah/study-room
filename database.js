...
async function pushAttendanceOnce(payload = {}) {
  ...
  if (lastSignatureAt && (Date.now() - lastSignatureAt) < 60000) return false;  
...
function startAttendanceAutoRefresh() {
  ...
  attendanceRefreshInterval = setInterval(() => {
    ...
    if (attendanceOpen || reportOpen) {
      clearTimeout(window.__attendanceRefreshLock);
      window.__attendanceRefreshLock = setTimeout(() => {
        refreshAttendanceViews({ silentReportRefresh: true });
      }, 5000);
...
function listenToGlobalBroadcastAlerts() {
  ...
  db.ref("bellUnreadCounts/" + idKey).off();
  db.ref("bellUnreadCounts/" + idKey).on("value", (snap) => {    const counts = snap.val() || 0;
    if (counts > 0) {
  ...
  activeBroadcastsRef.off();
  activePrivateMessagesRef.off();
...
db.ref("seats").off();
db.ref("seats").on("value", (snap) => {    const activeSeats = snap.val() || {};
  ...
  db.ref("attendance").limitToLast(50).on("value", () => {
...
function renderLessonsUI() {
  ...
  db.ref("lessons").get().then(async (snap) =>  {
